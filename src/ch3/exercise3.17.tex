...
the first equality holds only because the second expression does indeed converge to a finite number.
...
(Note: One may wonder whether the answer changes if $a \in \pi\mathbb{Q}$,
in which case the pole at $z = m\pi$ is removed when $ma \in \pi\mathbb{Z}$.
However, the residue $\sin am$ also vanishes in that case,
so the resulting answer $\tan \frac{a}{2}$ still holds!)
